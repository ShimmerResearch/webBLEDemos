...
export const MAG_SENS = {     // LSB per gauss (LIS2MDL mag — single fixed range)
  '50GA': 667,
};
...
export const ALIGN_MAG_3R = [      // DefaultAlignmentLIS2MDL (Shimmer Java API, SensorLIS2MDL.java)
  [-1, 0, 0],
  [0, -1, 0],
  [0, 0, -1],
];
...
/**
 * Raw counts -> device-frame physical units: full Ferraris form
 * C = R^-1 · K^-1 · (U − B). `offset` (B) is in RAW COUNTS and defaults to
 * zero, which matches the accel/gyro defaults; for the magnetometer the
 * offset is the hard-iron bias captured by a figure-8 routine and is the
 * dominant error term — never stream mag without it.
 */
export function calibrate(raw, sensitivity, alignment, offset = null) {
  const u = offset
    ? { x: raw.x - offset.x, y: raw.y - offset.y, z: raw.z - offset.z }
    : raw;
  const scaled = { x: u.x / sensitivity, y: u.y / sensitivity, z: u.z / sensitivity };
